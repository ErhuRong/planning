...
;;;; 
;;;; Code to load nonlin in LUCID COMMON LISP
;;;;
;;;; Author: B. Kettler

;;; The following three statements are required due to Lucid's
;;; nonstandard package names.
(in-package :user)
(rename-package (find-package :lisp) :lisp '(:common-lisp))
(rename-package (find-package :user) :user '(:common-lisp-user))
